...
int32_t DemoServiceProxy::RegisterCallback(const sptr<IRemoteObject>& callback)
{
    MessageParcel data;
    MessageParcel reply;
    MessageOption option;
    ...
    // 写入Callback远程对象
    if (callback == nullptr) {
        HiLogError(DEMO_LABEL, "[Client] RegisterCallback: callback is null!");
    ...
    if (result != ERR_NONE) {
        HiLogError(DEMO_LABEL, "[Client] RegisterCallback SendRequest failed: %{public}d", result);
        return result;
    }
