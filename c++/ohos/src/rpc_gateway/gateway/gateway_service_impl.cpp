...
GatewayServiceImpl::ClientDeathRecipient::ClientDeathRecipient(GatewayServiceImpl *gateway,
                                                                sptr<IRemoteObject> clientCallback)
    : gateway_(gateway), clientCallback_(clientCallback) {}

void GatewayServiceImpl::ClientDeathRecipient::OnRemoteDied(const wptr<IRemoteObject> &object)
{
...
void GatewayServiceImpl::DisconnectFromBackend()
{
...
bool GatewayServiceImpl::EnsureBackendCallbackRegistered()
{
    if (backendProxy_ == nullptr) {
...
void GatewayServiceImpl::EnsureBackendCallbackUnregistered()
{
    if (backendProxy_ != nullptr) {
        backendProxy_->UnregisterGatewayCallback();
...
int32_t GatewayServiceImpl::UnregisterClientCallback(const sptr<IEventCallback> &callback)
{
    if (callback == nullptr) {
        return ERR_INVALID_DATA;
    }
