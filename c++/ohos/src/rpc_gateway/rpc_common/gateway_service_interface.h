...
public:
    enum GatewayServiceCode {
        ...
        UNREGISTER_CLIENT_CALLBACK = 2,
        FORWARD_EVENT_TO_CLIENT = 3,
    };
    ...
    /**
     * @brief Unregister client's callback
     * @param callback The client's callback object to unregister
     * @return Error code, ERR_NONE on success
     */
    virtual int32_t UnregisterClientCallback(const sptr<IEventCallback> &callback) = 0;
