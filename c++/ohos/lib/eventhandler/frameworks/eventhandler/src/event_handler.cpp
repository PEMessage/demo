...
// void EventHandler::DistributeEvent(const InnerEvent::Pointer &event) __attribute__((no_sanitize("cfi"))) // PEM Modify
void EventHandler::DistributeEvent(const InnerEvent::Pointer &event) // PEM Modify // error: attributes are not allowed on a function-definition
{
    if (!event) {
...
typedef void (*ffrt_poller_cb)(void* data, uint32_t event); // PEM Modify // extra typedef cp from resourceschedule_ffrt/interfaces/kits/c/type_def.h 
