...
typedef struct {
    ...
    char *head;       // Pointer to the next byte to read
    char *tail;       // Pointer to the next byte to write
...
static size_t ringbuffer_read_internal(RingBuffer *rb, char *data, size_t count, bool update_buffer) {
    ...
    size_t bytes_read = 0;
    char *head = rb->head;  // Use local copy of head

    // First possible contiguous block (from head to end of buffer or wrap around)
    size_t chunk = (rb->buffer + rb->size) - head;
    if (chunk > count) chunk = count;
    memcpy(data, head, chunk);
    ...
    if (bytes_read < count) {
        ...
        memcpy(data + bytes_read, head, remaining);
        head += remaining;
...
size_t ringbuffer_write(RingBuffer *rb, const char *data, size_t count) {
    ...
    size_t chunk = (rb->buffer + rb->size) - rb->tail;
    if (chunk > count) chunk = count;
    memcpy(rb->tail, data, chunk);
    ...
    if (bytes_written < count) {
        ...
        memcpy(rb->tail, data + bytes_written, remaining);
        rb->tail += remaining;
...
const char* ringbuffer_at(const RingBuffer *rb, size_t index) {
    ...
    char *ptr = rb->head + index;
    if (ptr >= rb->buffer + rb->size) {
        ptr -= rb->size;
    }
    return ptr;
}
